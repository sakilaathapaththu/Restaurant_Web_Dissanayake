...
//     // must be exactly 9 digits after +94
//     if (!/^\d{9}$/.test(formData.customerPhone)) {
//       setError("Telephone number must be 9 digits (e.g., 712345678)");
//       return;
//     }
...
//       // 👉 Save in DB as 94xxxxxxxxx (no plus)
//       const orderData = {
//         userId: guestUserId,
//         ...formData,
...
//             <Typography
//               variant="h3"
//               sx={{ fontWeight: 700, color: "#2c1000", mb: 1 }}
//             >
//               Complete Your Order
//             </Typography>
...
//                         key={index}
//                         elevation={0}
//                         sx={{ borderRadius: 1, border: "1px solid #e8d5c4" }}
//                       >
//                         <CardContent sx={{ p: 2 }}>
//                           <Box
//                             sx={{
...
//                               <Typography
//                                 variant="body2"
//                                 sx={{ fontWeight: 600, color: "#fda021" }}
//                               >
...
//                   <Typography
//                     variant="h6"
//                     sx={{ fontWeight: 700, color: "#2c1000" }}
//                   >
//                     Total
//                   </Typography>
//                   <Typography
//                     variant="h6"
//                     sx={{ fontWeight: 700, color: "#fda021" }}
//                   >
...
//                       onChange={(e) => {
//                         // Allow only digits (max 9 after +94)
//                         const digitsOnly = e.target.value.replace(/\D/g, "");
//                         if (digitsOnly.length <= 9) {
//                           handleInputChange("customerPhone", digitsOnly);
//                         }
//                       }}
//                       required
//                       variant="outlined"
...
import api from "../Utils/api"; // your axios instance (already used elsewhere)
...
const CheckoutPage = () => {
  ...
  const [otpId, setOtpId] = useState("");       // token returned by backend
  const [otpCode, setOtpCode] = useState("");   // user-entered code
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msisdn = toSriLankaMSISDN(formData.customerPhone); // '94xxxxxxxxx' (no +)
      const { data } = await api.post("/otp/send", { phone: msisdn });
  ...
  // 2) verify OTP → on success, create order (your original logic)
  const handleVerifyOtp = async () => {
    if (!otpCode || !/^\d{4,6}$/.test(otpCode)) {
      setOtpErr("Enter the 6-digit OTP sent to your number");
      return;
    }
    ...
    try {
      ...
      const { data } = await api.post("/otp/verify", { otpId, code: otpCode });
      if (!data?.ok) {
        setOtpErr(data?.message || "Invalid/expired OTP. Try again.");
        return;
      }
      ...
      setOtpErr(err?.response?.data?.message || err.message || "OTP verification failed");
    } finally {
 ...
 const handleResendOtp = async () => {
    try {
      setOtpErr("");
      setOtpSending(true);
      const msisdn = toSriLankaMSISDN(formData.customerPhone);
      const { data } = await api.post("/otp/send", { phone: msisdn });
      if (data?.ok && data?.otpId) {
        setOtpId(data.otpId);
      } else {
        setOtpErr(data?.message || "Failed to resend OTP");
      }
    } catch (err) {
      setOtpErr(err?.response?.data?.message || err.message || "Failed to resend OTP");
    } finally {
      setOtpSending(false);
    }
  };

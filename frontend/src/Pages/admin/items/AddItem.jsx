...
import AddCircleOutlineIcon from "@mui/icons-material/AddCircleOutline";
import DeleteOutlineIcon from "@mui/icons-material/DeleteOutline";
import PhotoCameraIcon from "@mui/icons-material/PhotoCamera";               // ⭐️ added
import ResponsiveLayout from "../../../Components/Dashboard/ResponsiveLayout";
import API from "../../../Utils/api";
import { useNavigate } from "react-router-dom";
...
const ID_PREFIX = "ITEM";
const ID_MIN_WIDTH = 3;
const MAX_IMAGE_BYTES = 1.5 * 1024 * 1024;                                   // ⭐️ added (keep under backend limit)
const ALLOWED_MIME = ["image/jpeg", "image/png", "image/webp"];               // ⭐️ added
...
export default function AddItem() {
  ...
  // ⭐️ IMAGE state (added)
  const [imageBase64, setImageBase64] = useState("");         // data URI (e.g., "data:image/png;base64,....")
  ...
  const onImageChange = async (e) => {
    ...
    if (file.size > MAX_IMAGE_BYTES) {
      setErr("Image too large (max ~1.5MB)");
      return;
    }

    const reader = new FileReader();
    reader.onload = () => {
      setImageBase64(String(reader.result || "")); // data URI
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        const msg = e1?.response?.data?.error || "";
        if (/menuId|E11000|duplicate/i.test(msg)) {
          // regenerate and retry once (keeps your existing logic)
          await fetchNextMenuId();
          await API.post("/menu-items", { ...payload, menuId });
        } else {
          throw e1;
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Box component="form" onSubmit={onSubmit} noValidate>
              <Grid container spacing={2}>
                ...
                {/* ⭐️ Image uploader (added) */}
                <Grid item xs={12}>
                  <Typography variant="h6" fontWeight={700} sx={{ mb: 1 }}>Image (optional)</Typography>
                  ...
                  {imageBase64 ? (
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Allowed: JPEG / PNG / WEBP. Max ~1.5MB.
                    </Typography>
                  )}
                </Grid>

...
// const { sendText, normalizeSriPhone } = require("../services/SMS");

const { sendOrderSMS } = require("../services/orderSms");
  ...
  try {
    ...
    if (!userId || !customerName || !customerPhone || !address) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: userId, customerName, customerPhone, address",
      });
    }
    ...
    const calculatedGrandTotal =
      grandTotal ||
      calculatedTotalAmount + calculatedDeliveryFee + serviceCharge;
...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    const order = await Order.findByIdAndUpdate(theId, updateData, {
      new: true,
      runValidators: true,
    }).populate("items.foodId", "name image");
    ...
    // ✅ SMS via NotifyLK on confirm (keep same response shape "SMS")
    let SMS = { sent: false };
    if (order.status === "confirmed" && order.customerPhone) {
      try {
        await sendOrderSMS(order);        // sends a plain SMS using NotifyLK
        SMS = { sent: true };
      } catch (smsErr) {
        console.error("SMS send failed:", smsErr?.message || smsErr);
        SMS = { sent: false, error: String(smsErr?.message || smsErr) };
      }
    }
    ...
    return res.status(200).json({
      ...
      message: "Order status updated successfully",
      data: order,
      SMS, // frontend can check this (unchanged shape)
    });
  } catch (error) {
    console.error("Error updating order status:", error);
